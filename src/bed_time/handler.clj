(ns bed-time.handler
  (:require [compojure.core :refer [defroutes routes]]
            [bed-time.routes.home :refer [home-routes]]
            
            [bed-time.middleware
             :refer [development-middleware production-middleware]]
            [bed-time.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "bed_time.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (cronj/start! session/cleanup-job)
  (timbre/info "\n-=[ bed-time started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "bed-time is shutting down...")
  (stop-nrepl)
